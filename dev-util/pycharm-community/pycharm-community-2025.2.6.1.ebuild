...
SRC_URI="
amd64? ( https://download.jetbrains.com/python/pycharm-community-2025.2.6.1.tar.gz -> pycharm-community-2025.2.6.1-amd64.tar.gz )
arm64? ( https://download.jetbrains.com/python/pycharm-community-2025.2.6.1-aarch64.tar.gz -> pycharm-community-2025.2.6.1-aarch64.tar.gz )"
LICENSE="JetBrains"
SLOT="0"
KEYWORDS="*"
...
post_src_unpack() {
	if [ ! -d "$S" ]; then
	  ...
	  mv $(ls "${WORKDIR}") "pycharm-2025.2.6.1" || die
	fi
}
src_install() {
	patchelf --set-rpath '$ORIGIN' jbr/lib/jcef_helper jbr/lib/libjcef.so
	local dir="/opt/pycharm-community"
	local dst="${D}${dir}"
	insinto "${dir}"
	mv "${S}"/* "${dst}"
	local bundled_script_name="${PN%-*}.sh" # bundled script is always lowercase, and doesn't have -ultimate, -professional suffix.
	make_wrapper "pycharm-community" "${dir}/bin/$bundled_script_name" || die
	local svgfile="$(find ${dst}/bin -maxdepth 1 -iname '*.svg')"
	newicon $svgfile "pycharm-community.svg" || die
	local pngfile="$(find ${dst}/bin -maxdepth 1 -iname '*.png')"
	newicon $pngfile "pycharm-community.png" || die
	make_desktop_entry pycharm-community "PyCharm Community Edition" pycharm-community "Development;IDE;" || die
	if use system-java; then
	  rm -rf "$dst{jbr,jre{64}}" || die "Failed to remove bundled Java"
	fi
	if use sysctl; then
	  dodir /etc/sysctl.d
	  echo "fs.inotify.max_user_watches = 524288" > "${D}/etc/sysctl.d/30-pycharm-community-idea-inotify-watches.conf" || die
	fi
}
pkg_postinst() {
	xdg_pkg_postinst
