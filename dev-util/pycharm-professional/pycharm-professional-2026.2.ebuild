...
SRC_URI="
amd64? ( https://download.jetbrains.com/python/pycharm-2026.2.tar.gz -> pycharm-professional-2026.2-amd64.tar.gz )
arm64? ( https://download.jetbrains.com/python/pycharm-2026.2-aarch64.tar.gz -> pycharm-professional-2026.2-aarch64.tar.gz )"
LICENSE="JetBrains"
SLOT="0"
KEYWORDS="*"
...
"
S="${WORKDIR}/pycharm-2026.2"
post_src_unpack() {
	if [ ! -d "$S" ]; then
	  einfo "Renaming source directory to predictable name..."
	  mv $(ls "${WORKDIR}") "pycharm-2026.2" || die
	fi
}
src_install() {
