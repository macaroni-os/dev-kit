...
SRC_URI="
amd64? ( https://download.jetbrains.com/idea/ideaIC-2025.2.6.2.tar.gz -> idea-community-2025.2.6.2-amd64.tar.gz )
arm64? ( https://download.jetbrains.com/idea/ideaIC-2025.2.6.2-aarch64.tar.gz -> idea-community-2025.2.6.2-aarch64.tar.gz )"
LICENSE="JetBrains"
SLOT="0"
KEYWORDS="*"
...
"
S="${WORKDIR}/idea-IC-2025.2.6.2"
post_src_unpack() {
	if [ ! -d "$S" ]; then
	  einfo "Renaming source directory to predictable name..."
	  mv $(ls "${WORKDIR}") "idea-IC-2025.2.6.2" || die
	fi
}
src_install() {
	patchelf --set-rpath '$ORIGIN' jbr/lib/jcef_helper jbr/lib/libjcef.so
	local dir="/opt/idea-community"
	local dst="${D}${dir}"
	insinto "${dir}"
	mv "${S}"/* "${dst}"
	local bundled_script_name="${PN%-*}.sh" # bundled script is always lowercase, and doesn't have -ultimate, -professional suffix.
	make_wrapper "idea-community" "${dir}/bin/$bundled_script_name" || die
	local svgfile="$(find ${dst}/bin -maxdepth 1 -iname '*.svg')"
	newicon $svgfile "idea-community.svg" || die
	local pngfile="$(find ${dst}/bin -maxdepth 1 -iname '*.png')"
	newicon $pngfile "idea-community.png" || die
	make_desktop_entry idea-community "IntelliJ IDEA Community Edition" idea-community "Development;IDE;" || die
	if use system-java; then
	  rm -rf "$dst{jbr,jre{64}}" || die "Failed to remove bundled Java"
	fi
	if use sysctl; then
	  dodir /etc/sysctl.d
	  echo "fs.inotify.max_user_watches = 524288" > "${D}/etc/sysctl.d/30-idea-community-idea-inotify-watches.conf" || die
	fi
}
pkg_postinst() {
	xdg_pkg_postinst
