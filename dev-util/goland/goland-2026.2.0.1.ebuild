...
SRC_URI="
amd64? ( https://download.jetbrains.com/go/goland-2026.2.0.1.tar.gz -> goland-2026.2.0.1-amd64.tar.gz )
arm64? ( https://download.jetbrains.com/go/goland-2026.2.0.1-aarch64.tar.gz -> goland-2026.2.0.1-aarch64.tar.gz )"
LICENSE="JetBrains"
SLOT="0"
KEYWORDS="*"
...
"
S="${WORKDIR}/GoLand-2026.2.0.1"
post_src_unpack() {
	if [ ! -d "$S" ]; then
	  einfo "Renaming source directory to predictable name..."
	  mv $(ls "${WORKDIR}") "GoLand-2026.2.0.1" || die
	fi
}
src_install() {
