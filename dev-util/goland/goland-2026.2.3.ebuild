...
SRC_URI="
amd64? ( https://download.jetbrains.com/go/goland-2026.2.3.tar.gz -> goland-2026.2.3-amd64.tar.gz )
arm64? ( https://download.jetbrains.com/go/goland-2026.2.3-aarch64.tar.gz -> goland-2026.2.3-aarch64.tar.gz )"
LICENSE="JetBrains"
SLOT="0"
KEYWORDS="*"
...
"
S="${WORKDIR}/GoLand-2026.2.3"
post_src_unpack() {
	if [ ! -d "$S" ]; then
		einfo "Renaming source directory to predictable name..."
		mv $(ls "${WORKDIR}") "GoLand-2026.2.3" || die
	fi
}
src_install() {
